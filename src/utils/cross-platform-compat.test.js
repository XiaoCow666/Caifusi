 ...
 *     （如 `set -e`），`set VAR=value` 不会按 CMD 语义导出环境变量。
 *     正确语法是 `VAR=value command` 或 `export VAR=value && command`。
 *   因此原脚本在 bash 中：`set WDS_SOCKET_HOST=localhost` 不报错但无效，
 *   `&&` 后 react-scripts 仍启动，然而 HOST / DANGEROUSLY_DISABLE_HOST_CHECK
 *   均未生效（推断，未经 macOS/Linux 实测）。
 *
 * 修复：将环境变量移至 .env.development。react-scripts（CRA）启动时用
 *   dotenv 加载 .env.development 到 process.env，与 shell 无关。
 *
 * 安全收紧（2026-09-16）：迁移使变量在所有平台生效，为避免扩大
 *   HOST=0.0.0.0 + DANGEROUSLY_DISABLE_HOST_CHECK=true 的暴露面，
 *   默认改为 HOST=localhost，且不设置禁用 Host 检查的配置。
 *   需要局域网访问时另行显式配置。
 *
...
describe('start 脚本跨平台兼容', () => {
  test('package.json start 脚本不含 Windows CMD set 语法', () => {
    ...
    // bash 中 `set VAR=value` 不会按 CMD 语义设置环境变量，必须禁止
...
describe('.env.development 环境变量', () => {
  ...
  test('默认监听 localhost（HOST=localhost）', () => {
    const content = fs.readFileSync(envPath, 'utf8');
    expect(content).toMatch(/^HOST=localhost$/m);
  });

  test('未设置禁用 Host 检查的配置（保留 Host 校验）', () => {
    const content = fs.readFileSync(envPath, 'utf8');
    expect(content).not.toMatch(/^DANGEROUSLY_DISABLE_HOST_CHECK=/m);
  });
