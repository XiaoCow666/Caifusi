...
5. **DEV_MODE 的预期行为**：Dashboard 认证装饰器中有 DEV_MODE 绕过逻辑，但是否应在开发环境默认启用？
...
### 改进二：注册 auth 路由并启用 DEV_MODE，打通前端到后端的完整链路（预计 1 天）
...
2. 在 `config.py` 中添加 `DEV_MODE = os.environ.get('DEV_MODE', 'true').lower() == 'true'`，使开发环境默认启用 DEV_MODE
3. 确保前端 `api.js` 在调用 Dashboard API 时携带正确的请求头（即使是 mock token，DEV_MODE 下也会被绕过）
4. 验证前端 Dashboard 页面能正常获取后端数据

**风险**：低。注册已有路由、添加配置项，不改变业务逻辑。需注意 DEV_MODE 在生产环境必须设为 false。

**验证**：启动前后端，在前端登录后访问 Dashboard，确认能正常显示数据（即使是测试用户的空数据）。
...
对于接手者来说，**第一优先级应该是"让它能完整跑通"**——修复依赖问题、注册 auth 路由、启用 DEV_MODE、验证 AI 对话实际效果。在此基础上，再考虑数据持久化、真实认证、测试覆盖等工程化改进。
